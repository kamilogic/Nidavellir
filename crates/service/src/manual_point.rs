...
impl ManualPointHandle {
    ...
    pub fn apply(
        ...
        store
            .arm_boot_flag(&BootFlag::new(intent, "manual_diagnostic_point"))
            .map_err(|error| {
                format!("Manual point: failed to arm Safe Loop before write: {error}")
            })?;
        ...
        self.status = ManualDiagnosticPointStatus {
            active: true,
            ...
            verified: true,
            note: "Temporary anchored curve is active. Start Game Trace before launching the workload.".into(),
        };
        Ok(self.status())
    }
...
fn reset_hardware() -> Result<(), String> {
...
fn reset_and_disarm(store: &SafeLoopStore) -> Result<(), String> {
    reset_hardware()?;
    crate::gpu_apply::clear_applied();
...
mod tests {
    ...
    fn mark_reset_clears_the_selected_point() {
        ...
        handle.status.target_mhz = Some(1800);
        handle.mark_reset();
